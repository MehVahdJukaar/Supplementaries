...
public class BombSmokeParticle extends TextureSheetParticle {
    private BombSmokeParticle(ClientLevel world, double x, double y, double z, double motionX, double motionY, double motionZ) {
        ...
        this.xd = motionX;
        this.yd = motionY + 0.01 + (double)(this.random.nextFloat() / 2000.0F);
        this.zd = motionZ;
    }
    ...
    public void tick() {
        ...
        if (this.age++ < this.lifetime && !(this.alpha <= 0.0F)) {
            this.xd += this.random.nextFloat() / 3500.0F * (float)(this.random.nextBoolean() ? 1 : -1);
            this.zd += this.random.nextFloat() / 3500.0F * (float)(this.random.nextBoolean() ? 1 : -1);
