...
public class MultiLineEditBoxWidget extends Widget {
    ...
    public MultiLineEditBoxWidget(Minecraft mc, int x, int y, int width, int height) {
        super(x, y, width, height, new StringTextComponent("hhhhh"));
        this.minecraft = mc;
        this.font = mc.font;
        this.x1 = x + width;
        this.y1 = y + height;

        this.pageEdit = new TextInputUtil(this::getText, this::setText,
    ...
    public boolean canConsumeInput() {
        return this.isFocused() && this.active;
    }
    ...
    @Override
    public void render(MatrixStack poseStack, int mouseX, int museY, float partialTicks) {
        if (this.visible) {
            DisplayCache displayCache = this.getDisplayCache();
    ...
    private void renderCursor(MatrixStack poseStack, Pos2i pos2i, boolean p_98111_) {
        if (this.frameTick / 6 % 2 == 0) {
            ...
            if (!p_98111_) {
                AbstractGui.fill(poseStack, pos2i.x, pos2i.y - 1, pos2i.x + 1, pos2i.y + 9, -16777216);
            } else {
    ...
    private void selectWord(int p_98142_) {
        ...
        this.pageEdit.setSelectionRange(CharacterManager.getWordPosition(s, -1, p_98142_, false), CharacterManager.getWordPosition(s, 1, p_98142_, false));
    }
    ...
    private DisplayCache rebuildDisplayCache() {
        ...
        if (s.isEmpty()) {
            ...
            CharacterManager stringsplitter = this.font.getSplitter();
    ...
    static class DisplayCache {
        static final DisplayCache EMPTY = new DisplayCache("", new Pos2i(0, 0), true, new int[]{0}, new LineInfo[]{new LineInfo(Style.EMPTY, "", 0, 0)}, new Rectangle2d[0]);
    ...
    static class LineInfo {
        ...
        public LineInfo(Style style, String contents, int x, int y) {
            ...
            this.x = x;
            this.y = y;
            this.asComponent = (new StringTextComponent(contents)).setStyle(style);
        }
    }
