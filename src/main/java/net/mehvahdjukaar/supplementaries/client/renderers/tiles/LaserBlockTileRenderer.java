...
public class LaserBlockTileRenderer extends TileEntityRenderer<LaserBlockTile> {
    ...
    public void render(LaserBlockTile tile, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn,
        ...
        if (tile.canEmit()) {
            ...
            if (lenght == 0)
                return;
            TextureAtlasSprite sprite_o = Minecraft.getInstance().getAtlasSpriteGetter(AtlasTexture.LOCATION_BLOCKS_TEXTURE).apply(Textures.LASER_OVERLAY_TEXTURE);
            TextureAtlasSprite sprite = Minecraft.getInstance().getAtlasSpriteGetter(AtlasTexture.LOCATION_BLOCKS_TEXTURE).apply(Textures.LASER_BEAM_TEXTURE);
            ...
            if (lenght == MAXLENGHT + 1) {
                ...
                matrixStackIn.push();
                IVertexBuilder builder1 = bufferIn.getBuffer(RenderType.getLightning());
                TextureAtlasSprite sprite1 = Minecraft.getInstance().getAtlasSpriteGetter(AtlasTexture.LOCATION_BLOCKS_TEXTURE).apply(Textures.LASER_BEAM_END_TEXTURE);
