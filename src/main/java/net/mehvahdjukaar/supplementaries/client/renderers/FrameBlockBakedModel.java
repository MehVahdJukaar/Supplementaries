...
public class FrameBlockBakedModel implements IDynamicBakedModel {
    ...
    public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, @Nonnull Random rand, @Nonnull IModelData extraData) {
        ...
        if(mimic == null || (mimic.isAir())){
            IBakedModel model = Minecraft.getInstance().getBlockRenderer().getBlockModelShaper().getBlockModel(state.setValue(FrameBlock.TILE,0));
        ...
        IBakedModel model = Minecraft.getInstance().getBlockRenderer().getBlockModelShaper().getBlockModel(state.setValue(FrameBlock.TILE,2));
        try {
            quads.addAll(model.getQuads(mimic, side, rand, EmptyModelData.INSTANCE));
        } catch (Exception ignored) {}
