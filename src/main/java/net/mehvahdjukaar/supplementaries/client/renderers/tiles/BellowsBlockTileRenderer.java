...
public class BellowsBlockTileRenderer implements BlockEntityRenderer<BellowsBlockTile> {
    ...
    @Override
    public void render(BellowsBlockTile tile, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int combinedLightIn,
                       int combinedOverlayIn) {
        VertexConsumer builder = Materials.BELLOWS_MATERIAL.buffer(buffer, RenderType::entitySolid);
        ...
        poseStack.mulPose(Const.rot(dir.getOpposite()));
        poseStack.mulPose(Const.XN90);
        poseStack.mulPose(Const.Z180);
