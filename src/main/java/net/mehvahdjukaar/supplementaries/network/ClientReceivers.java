...
public class ClientReceivers {
    ...
    public static void handleSyncTradesPacket(ClientBoundSyncTradesPacket message) {
        withPlayerDo(p -> {
            ...
            if (message.getContainerId() == container.containerId && container instanceof RedMerchantContainer merchantContainer) {
                (merchantContainer).setOffers(new MerchantOffers(message.offers.createTag()));
                (merchantContainer).setXp(message.getVillagerXp());
                (merchantContainer).setMerchantLevel(message.getVillagerLevel());
                (merchantContainer).setShowProgressBar(message.isShowProgress());
                (merchantContainer).setCanRestock(message.isCanRestock());
            }
        });
    }
}
