...
public class AdventurerMapsHandler {
    ...
    private static final List<Structure<?>> randomMapPool = Arrays.asList(Structure.SHIPWRECK, Structure.RUINED_PORTAL,
            Structure.SWAMP_HUT, Structure.BASTION_REMNANT, Structure.JUNGLE_TEMPLE, Structure.DESERT_PYRAMID,
            Structure.PILLAGER_OUTPOST, Structure.MINESHAFT, Structure.OCEAN_RUIN, Structure.IGLOO, Structure.END_CITY);
    ...
    private static class AdventureMapTrade implements VillagerTrades.ITrade {
        ...
        private ItemStack createMap(World world, BlockPos pos) {
            Structure<?> structure = ForgeRegistries.STRUCTURE_FEATURES.getValue(tradeData.structure);
