...
public class CoopResolverHelper {
     ...
     * <p>
     * Effective cap: {@code 12 × (1 + contributingCooperators.size())}. The boosted
    ...
    public static boolean gateResolve(boolean originalResult, BlockPos pistonPos,
        ...
        for (BlockPos coop : state.cooperatingPistons) {
            if (coop.equals(pistonPos)) continue;
            BlockPos start = coop.relative(state.pistonDirection, offset);
            if (toPush.contains(start)) contributing.add(coop);
        }
