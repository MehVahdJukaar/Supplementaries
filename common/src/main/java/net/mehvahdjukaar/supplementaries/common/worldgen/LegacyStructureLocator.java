...
public class LegacyStructureLocator {
    ...
    public static List<LocatedStruct> findNearestMapFeatures(
        ...
        Supplementaries.LOGGER.info("Searching for closest structure among {} from pos {}",
                Arrays.toString(selectedTargets.stream().map(Holder::getRegisteredName).toArray()), pos);
        ...
        List<LocatedStructure> newStr = foundStructures.stream().map(s ->
                LocatedStructure.relativeTo(s.pos, s.structure, s.start, pos)).toList();

        Supplementaries.LOGGER.info("\n Structure locator found {} structures: \n{}", foundStructures.size(),
                String.join("\n", newStr.stream().map(LocatedStructure::toString).toArray(CharSequence[]::new)));
