...
public class SliceMapItem extends EmptyMapItem {
    ...
    public static MaterialColor getCutoffColor(BlockPos pos, BlockGetter level) {
        ...
        return (pos.getX() + pos.getZ()) % 2 == 0 ? MaterialColor.NONE : WeatheredMap.ANTIQUE_LIGHT;
    }
    ...
    public static boolean canPlayerSee(int targetY, Entity entity) {
        ...
        while (p.getY() != targetY) {
            if (level.getBlockState(p).getMapColor(level, p) != MaterialColor.NONE) {
                return false;
            }
