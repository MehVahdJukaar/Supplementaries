...
public class PistonAttemptsTracker {
    ...
    private static final Map<BlockPos, ClientCooperation> CLIENT_COOPERATORS_BY_PISTON = new HashMap<>();
    ...
    public static void markAttempting(ServerLevel level, BlockPos pos, Direction direction, boolean extending) {
        long now = level.getGameTime();
        Map<BlockPos, Attempt> attempts = SERVER_ATTEMPTS.computeIfAbsent(level, l -> new HashMap<>());
        attempts.values().removeIf(a -> !a.isFresh(now));
    ...
    public static Set<BlockPos> getCooperators(ServerLevel level, BlockPos pistonPos, Direction direction, boolean extending) {
        ...
        for (Map.Entry<BlockPos, Attempt> entry : attempts.entrySet()) {
            ...
            if (!attempt.isFresh(now) || attempt.direction() != direction || attempt.extending() != extending)
                continue;
    ...
    public static void onClientCooperatorsReceived(Level level, BlockPos pos, Set<BlockPos> cooperators) {
        long now = level.getGameTime();
        CLIENT_COOPERATORS_BY_PISTON.values().removeIf(c -> now - c.receivedTick() > 1);
        CLIENT_COOPERATORS_BY_PISTON.put(pos, new ClientCooperation(cooperators, now));
    }

    @Nullable
    public static Set<BlockPos> takeClientCooperators(Level level, BlockPos pos) {
        ClientCooperation pending = CLIENT_COOPERATORS_BY_PISTON.remove(pos);
    ...
    private static boolean canReachSameStructure(BlockPos candidate, BlockPos pistonPos, Direction.Axis pushAxis) {
        Vec3i delta = candidate.subtract(pistonPos);
        int distanceAlongPush = Math.abs(pushAxis.choose(delta.getX(), delta.getY(), delta.getZ()));
        if (distanceAlongPush >= PistonStructureResolver.MAX_PUSH_DEPTH) return false;

        int acrossX = pushAxis == Direction.Axis.X ? 0 : Math.abs(delta.getX());
        int acrossY = pushAxis == Direction.Axis.Y ? 0 : Math.abs(delta.getY());
        int acrossZ = pushAxis == Direction.Axis.Z ? 0 : Math.abs(delta.getZ());
        int distanceAcrossPush = Math.max(acrossX, Math.max(acrossY, acrossZ));
        return distanceAcrossPush > 0 && distanceAcrossPush <= PistonStructureResolver.MAX_PUSH_DEPTH;
    }
}
